...
from aifo.dlup._image import Resampling
from aifo.dlup.annotations import SlideAnnotations
from aifo.dlup.utils.backends import ImageBackend
from aifo.dlup.data.dataset import SlideDataset
from aifo.dlup.tiling import TilingMode, GridOrder
...
def get_relative_scaling(slide_image: SlideImage, native_mpp_at_inference: float) -> tuple[float, tuple[int, int], tuple[float, float]]:
...
def get_itsp_score(image_dataset: SlideDataset) -> tuple[float, float, float, float]:
    ...
    for sample in image_dataset:
        ...
        stroma_compartment, tumor_compartment, other_compartment = get_class_pixels(image_array)
...
def get_image_dataset(wsi_path: Path, annotation_path: Path, native_mpp_for_inference: float, tile_size: tuple[int, int], **kwargs) -> SlideDataset:
...
def get_inference_dataset(wsi_path: Path, inference_file: Path, annotation_path: Path, native_mpp_for_inference: float, tile_size: tuple[int, int], **kwargs) -> SlideDataset:
    annotations = SlideAnnotations.from_geojson(annotation_path)
    slide_image = SlideImage.from_file_path(wsi_path, **kwargs)
    available_labels = annotations.available_classes
    for a_class in available_labels:
...
def itsp_computer(
    ...
    for index, image, inference_image, annotation, itsp_score in data_rubric:
        ...
        image_dataset = get_image_dataset(wsi_path, slide_annotation_path, native_mpp_for_inference, tile_size, **kwargs)
        try:
            prediction_slide_dataset = get_inference_dataset(wsi_path, inference_file, slide_annotation_path, native_mpp_for_inference, tile_size, **kwargs)
        ...
        if render_images:
            ...
            render_visualization(image_dataset, prediction_slide_dataset, output_path=output_path, slide_id=slide_id, human_itsp_score=human_itsp_score, ai_itsp_score=itsp)
